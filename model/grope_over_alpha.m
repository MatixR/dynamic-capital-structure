...
% 	dbstop if warning
	...
	fid_output = fopen('grope_over_alpha.csv', 'w');

	incr = 0.001;
	
	for alpha_next = (alpha+incr):incr:0.95
		...
		coupons(2) = coupons(1) * 1.2;
		
		fprintf(fid_output, '%.4f\t%.12f\t%.12f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n', ...
			alpha, X_D(1,1), X_D(1,2), X_U(1,1), X_U(1,2), capital(1), coupons(1), coupons(2) );
