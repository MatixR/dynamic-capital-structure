function params = arrow_default_params(state0)
	...
	if (XD_this(2) <= XD_this(1)) && (XU_this(2) <= XU_this(1))
		...
		params = arrow_default_params_case1(params);

	elseif (XD_this(2) <= XD_this(1)) && (XU_this(1) <= XU_this(2))

		params = arrow_default_params_case2(params);

	elseif (XD_this(1) <= XD_this(2)) && (XU_this(2) <= XU_this(1))

		params = arrow_default_params_case3(params);

	elseif (XD_this(1) <= XD_this(2)) && (XU_this(1) <= XU_this(2))

		params = arrow_default_params_case4(params);
...
function params = arrow_default_params_case1(params)
	...
	% Ax = b
	% x = [g11 (1:2), g12 (3:4), h11 (5:8), h12 (9:12), s21 (13:14), s22 (15:16)]
	...
	A(9,13:14) = XD2_j;
	b(9) = - lambda_hat(2) / (riskfree(2) + lambda_hat(2));
...
function params = arrow_default_params_case2(params)
	...
	% Boundary condition (e)
	A(5,13:14) = XD2_j;
	b(5) = - lambda_hat(2) / (riskfree(2) + lambda_hat(2));
	
	% Boundary condition (f)
	A(6,5:8) = eps .* XD1_k;
	A(6,13:14) = - XD1_j;
	b(6) = lambda_hat(2) / (riskfree(2) + lambda_hat(2));
...
function params = arrow_default_params_case3(params)
	...
	% Boundary condition (g)
	A(7,15:16) = XD1_l;
	b(7) = - lambda_hat(1) / (riskfree(1) + lambda_hat(1));
	
	% Boundary condition (h)
	A(8,15:16) = XD2_l;
	A(8,9:12)  = - XD2_k;
	b(8) = - lambda_hat(1) / (riskfree(1) + lambda_hat(1));
...
function params = arrow_default_params_case4(params)
	...
	% Boundary condition (e)
	A(5,15:16)  = XD1_l;
	b(5) = - lambda_hat(1) / (riskfree(1) + lambda_hat(1));
